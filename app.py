...
has_comorbidities = st.radio(
    "האם יש מחלות רקע משמעותיות או פגיעה בתפקוד?",
    ["כן", "לא"], horizontal=True, index=None
)

ecog = None
if has_comorbidities == "כן":
    ecog = st.selectbox("ECOG Performance Status", [
        "",
        "0 — מסוגל לבצע את כל הפעילויות כרגיל",
        "1 — מסוגל לפעילות קלה, לא לפעילות גופנית מאומצת",
        "2 — מסוגל לטיפול עצמי, לא מסוגל לעבוד",
        "3 — מסוגל לחלק מהטיפול העצמי; מרבה בשהות במיטה/כיסא",
        "4 — לא מסוגל לטיפול עצמי; כל הזמן במיטה/כיסא"
    ])
...
hematologic_exclusions = st.multiselect(
    "הדרות המטולוגיות (ניתן לסמן יותר מאחד)",
    [
...
labs_available = st.radio("האם יש תוצאות בדיקות זמינות?",
                          ["כן", "לא"], horizontal=True, index=None)

lab_file = None
if labs_available == "כן":
    lab_file = st.file_uploader(
        "העלאת תמונה / פרינטסקרין של בדיקות (JPG / PNG / PDF)",
        type=["jpg", "jpeg", "png", "pdf"]
    )

    # ── חילוץ אוטומטי מתמונה ────────────────────────────────────────────
    if lab_file is not None and lab_file.type in ["image/jpeg", "image/png", "image/jpg"]:
        st.info("📷 תמונה הועלתה — ניתן לחלץ ערכים אוטומטית")

        _api_key = st.secrets.get("ANTHROPIC_API_KEY", "")

        if st.button("🤖 חלץ ערכים אוטומטית מהתמונה", type="primary", use_container_width=True):
            if not _api_key:
                st.error("מפתח API לא מוגדר — פנה למנהל המערכת")
            else:
                with st.spinner("Claude קורא את בדיקות המעבדה... (5-15 שניות)"):
                    image_bytes = lab_file.read()
                    mime = lab_file.type
                    result = extract_lab_values(image_bytes, _api_key, mime)
                ...
                if result["success"]:
                    ...
                    for f in LAB_FIELDS:
                        val = result.get(f)
                        if val is not None:
                            st.session_state[f"lab_{f}"] = float(val)
                            filled += 1

                    notes = result.get("notes")
                    st.success(f"✅ חולצו {filled} ערכים בהצלחה!")
                    if notes:
                        st.warning(f"הערות: {notes}")
                    st.rerun()
                else:
                    st.error(f"❌ {result['error']}")

    elif lab_file is not None and lab_file.type == "application/pdf":
        st.info("PDF הועלה — חילוץ אוטומטי תומך בתמונות (JPG/PNG). עבור PDF, נא למלא ידנית.")
